...
function Form({ patients, setPatients, patient, setPatient }) {
  ...
  useEffect(() => {
    ...
    if (Object.keys(patient).length > 0) {
      const { petName, ownerName, email, releaseDate, symptoms } = patient;
      setPetName(petName);
  ...
  const handleSubmit = (e) => {
    ...
    if (patient.id) {
      ...
      const patientsEdited = patients.map((patient) =>
        patient.id === patientObj.id ? patientObj : patient
      );
    ...
    <div className="md:w-1/2 lg:w-2/5 mx-3">
      ...
      >
        ...
        <input
          ...
          value={patient.id ? "Save Changes" : "Add New Patient"}
        />
      </form>
    </div>
  );
